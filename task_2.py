    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if line:
            try:
                ...
                for g in grades_list:
                    ...
                    if g:
                        try:
                            ...
                            print(
                                f"Строка {line_num}: '{g}' не является числом - пропускаем"
                            )
                            valid = False
                            break
                    else:
                        print(f"Строка {line_num}: пустая оценка - пропускаем")
                        valid = False
                        break

                if not valid or not grades:
                    continue
                ...
                print(
                    f"{len(students)}. {name} - оценки: {grades}, средний балл: {avg}"
                )
